 ...
 * @template R Type of the read value. Use 'any' (or omit) if this contains multiple different types.
 * @preserve (Part of the public API)
 */export function writeSymbolEx2(symbolNames,values,options,callback){let symbolNameArr,symbolValueArr,symbolOptions=options?.symbolOptions??null,requestOptions=options?.requestOptions??null,symbolOptionsArr=null;if(Array.isArray(symbolNames)){if(!Array.isArray(values)||symbolNames.length!==values.length)return Callback.callSafeEx(callback,null,{error:Errors.E_PARAMETER_INVALID,details:{code:Errors.E_PARAMETER_INVALID,message:Errors[Errors.E_PARAMETER_INVALID],reason:"Symbol array length does not match value array length",domain:"TcHmi.Server"}}),null;if(symbolNameArr=symbolNames,symbolValueArr=values,symbolOptions){if(isParameterTypeInvalid(symbolOptions,"options.symbolOptions",{type:"object",expectArray:!0,required:"valueNeeded",minArrayLength:symbolNameArr.length},"TcHmi.Server",callback))return null;symbolOptionsArr=symbolOptions}}else if(symbolNameArr=[symbolNames],symbolValueArr=[values],symbolOptions){if(isParameterTypeInvalid(symbolOptions,"options.symbolOptions",{type:"object",expectArray:!1,required:"valueNeeded"},"TcHmi.Server",callback))return null;symbolOptionsArr=[symbolOptions]}const callstackLinker=createTask("Server.write>"+(symbolNameArr?.[0]??"unknown")),request={requestType:"ReadWrite",commands:[]};for(let i=0;i<symbolNameArr.length;i++){let strSymbolName=symbolNameArr[i],SymbolValue=symbolValueArr[i];if(isParameterTypeInvalid(strSymbolName,"symbolNames",{type:"string",required:"valueNeeded",minStringLength:1},"TcHmi.Server",callback))return null;let command={commandOptions:["SendErrorMessage","SendWriteValue"],symbol:strSymbolName,writeValue:SymbolValue};if(symbolOptionsArr){let symbolOptions=symbolOptionsArr[i];symbolOptions&&symbolOptions.version&&(command.version=symbolOptions.version)}request.commands.push(command)}return serverManager.requestEx(request,requestOptions,handleResponse({completed:data=>{callstackLinker.run(()=>{Callback.callSafeEx(callback,null,data)})}}))}
/**
 ...
 * @template R Type of the read value. Use 'any' (or omit) if this contains multiple different types.
 * @preserve (Part of the public API)
 */export function subscribeSymbolEx2(symbolNames,writeValues,interval,options,callback){let symbolNameArr,symbolValueArr,symbolOptions=options?.symbolOptions??null,requestOptions=options?.requestOptions??null,symbolOptionsArr=null;if(Array.isArray(symbolNames)){if(void 0!==writeValues){if(!Array.isArray(writeValues)||symbolNames.length!==writeValues.length)return Callback.callSafeEx(callback,null,{error:Errors.E_PARAMETER_INVALID,details:{code:Errors.E_PARAMETER_INVALID,message:Errors[Errors.E_PARAMETER_INVALID],reason:"Symbol array length does not match value array length",domain:"TcHmi.Server"}}),null;Array.isArray(writeValues)&&(symbolValueArr=writeValues)}if(symbolNameArr=symbolNames,symbolOptions){if(isParameterTypeInvalid(symbolOptions,"options.symbolOptions",{type:"object",expectArray:!0,required:"valueNeeded",minArrayLength:symbolNameArr.length},"TcHmi.Server",callback))return null;symbolOptionsArr=symbolOptions}}else if(symbolNameArr=[symbolNames],void 0!==writeValues&&(symbolValueArr=[writeValues]),symbolOptions){if(isParameterTypeInvalid(symbolOptions,"options.symbolOptions",{type:"object",expectArray:!1,required:"valueNeeded"},"TcHmi.Server",callback))return null;symbolOptionsArr=[symbolOptions]}let commands=[];for(let i=0;i<symbolNameArr.length;i++){let strSymbolName=symbolNameArr[i],symbolValue=symbolValueArr?.[i];if(isParameterTypeInvalid(strSymbolName,"symbolNames",{type:"string",required:"valueNeeded",minStringLength:1},"TcHmi.Server",callback))return null;let command={commandOptions:["SendErrorMessage","SendWriteValue"],symbol:strSymbolName};if(void 0!==symbolValue&&(command.writeValue=symbolValue),symbolOptionsArr){let symbolOptions=symbolOptionsArr[i];symbolOptions&&symbolOptions.version&&(command.version=symbolOptions.version)}commands.push(command)}return subscribeEx(commands,interval,requestOptions,handleResponse({completed:data=>{Callback.callSafeEx(callback,null,data)}}))}export function resolveSymbolSchema(symbolName,callback){resolveSymbolSchemaEx(symbolName,null,callback)}export function resolveSymbolSchemaEx(symbolName,options,callback){const version=options?.version??null,symbolMetaData=Data.Caches.serverSymbolMetaDataCache.get(symbolName);if(symbolMetaData){if(!symbolMetaData.ListSymbols?.SCHEMA)return void Callback.callSafeEx(callback,null,{error:Errors.E_SYMBOL_RESOLVE_SCHEMA,details:{code:Errors.E_SYMBOL_RESOLVE_SCHEMA,message:Errors[Errors.E_SYMBOL_RESOLVE_SCHEMA],domain:"TcHmi.Server"}});let res=TypeHelper.__resolveRawSchema(symbolMetaData.ListSymbols.SCHEMA,null,new Map,version);res.error===Errors.NONE&&res.schema?Callback.callSafeEx(callback,null,{error:Errors.NONE,schema:tchmi_clone_object(res.schema)}):Callback.callSafeEx(callback,null,{error:res.error,details:res.details})}else{const{name:baseName,pathTokens:basePathTokens}=resolveServerSymbolNameParts(symbolName);let baseSchemaCacheEntry=Data.Caches.serverSymbolMetaDataCache.get(baseName);if(baseSchemaCacheEntry){if(!baseSchemaCacheEntry.ListSymbols?.SCHEMA)return void Callback.callSafeEx(callback,null,{error:Errors.ERROR,details:{code:Errors.ERROR,message:Errors[Errors.ERROR]}});if(baseSchemaCacheEntry.error!==Errors.NONE)return void Callback.callSafeEx(callback,null,{error:baseSchemaCacheEntry.error,details:baseSchemaCacheEntry.details});{let baseSchema=baseSchemaCacheEntry.ListSymbols.SCHEMA;if(baseSchema){let res=TypeHelper.__resolveRawSchema(baseSchema,null,new Map);res.error===Errors.NONE&&res.schema?TypeHelper.__resolveSubSchema(res.schema,basePathTokens,function(data){data&&data.error===Errors.NONE?Callback.callSafeEx(callback,null,{error:Errors.NONE,schema:tchmi_clone_object(data.schema)}):Callback.callSafeEx(callback,null,{error:data.error,details:data.details})}):Callback.callSafeEx(callback,null,{error:res.error,details:res.details})}}}}}export function resolveSymbolMetaData(symbolName,callback){let data=Data.Caches.serverSymbolMetaDataCache.get(symbolName);data?Callback.callSafeEx(callback,null,data):Callback.callSafeEx(callback,null,{error:Errors.E_SYMBOL_RESOLVE_META_DATA,details:{code:Errors.E_SYMBOL_RESOLVE_META_DATA,message:Errors[Errors.E_SYMBOL_RESOLVE_META_DATA],domain:"TcHmi.Server"}})}
/**
 ...
 * @returns returns a boolean if the logout was called
 * @preserve (Part of the public API)
 */export function forceLogoutEx(userName,requestOptions,callback){let res=!1;return void 0!==userName&&""!==userName||(userName=null),res=serverManager.forceLogout(userName,requestOptions,callback),res}export function handleResponse(args){return data=>{if(data.error!==Errors.NONE)return Callback.callSafeEx(args.error,null,data),void Callback.callSafeEx(args.completed,null,data);let response=data.response,responseCommandIndices=data.responseCommandIndices;if(data.results){let hasCommandError=!1;for(let result of data.results)result.error!==Errors.NONE&&(hasCommandError=!0);hasCommandError?Callback.callSafeEx(args.error,null,data):Callback.callSafeEx(args.success,null,data),Callback.callSafeEx(args.completed,null,data)}else{if(!response)return Callback.callSafeEx(args.error,null,{error:Errors.E_SERVER_RESPONSE_MISSING,details:{code:Errors.E_SERVER_RESPONSE_MISSING,message:Errors[Errors.E_SERVER_RESPONSE_MISSING],reason:"Missing response from server.",domain:"TcHmi.Server"}}),void Callback.callSafeEx(args.completed,null,{error:Errors.E_SERVER_RESPONSE_MISSING,details:{code:Errors.E_SERVER_RESPONSE_MISSING,message:Errors[Errors.E_SERVER_RESPONSE_MISSING],reason:"Missing response from server.",domain:"TcHmi.Server"}});if(response.error){const res={error:Errors.E_SERVER_RESPONSE_ERROR,details:{code:Errors.E_SERVER_RESPONSE_ERROR,message:Errors[Errors.E_SERVER_RESPONSE_ERROR],reason:"Error in response from server with id: "+response.id,domain:"TcHmi.Server",errors:[response.error]}};return void 0!==response&&(res.response=response),void 0!==responseCommandIndices&&(res.responseCommandIndices=responseCommandIndices),Callback.callSafeEx(args.error,null,res),void Callback.callSafeEx(args.completed,null,res)}let commands=response.commands;if(!commands||0===commands.length){const res={error:Errors.E_SERVER_COMMANDS_MISSING,details:{code:Errors.E_SERVER_COMMANDS_MISSING,message:Errors[Errors.E_SERVER_COMMANDS_MISSING],reason:"Missing commands in response from server with id: "+response.id,domain:"TcHmi.Server"}};return void 0!==response&&(res.response=response),void 0!==responseCommandIndices&&(res.responseCommandIndices=responseCommandIndices),Callback.callSafeEx(args.error,null,res),void Callback.callSafeEx(args.completed,null,res)}let hasCommandError=!1,commandErrors=[];data.results=[];for(const[index,command]of commands.entries())if(!data.responseCommandIndices||data.responseCommandIndices.includes(index))if(command.error){hasCommandError=!0,commandErrors.push({code:Errors.E_SERVER_COMMAND_ERROR,message:Errors[Errors.E_SERVER_COMMAND_ERROR],reason:'Error in command for symbol: "'+command.symbol+'" in response from server with id: '+response.id,domain:"TcHmi.Server",errors:[command.error]});const res={error:Errors.E_SERVER_COMMAND_ERROR,details:{code:Errors.E_SERVER_COMMAND_ERROR,message:Errors[Errors.E_SERVER_COMMAND_ERROR],reason:'Error in command for symbol: "'+command.symbol+'" in response from server with id: '+response.id,domain:"TcHmi.Server",errors:[command.error]},symbol:command.symbol};void 0!==data.response&&(res.response=data.response),void 0!==data.responseCommandIndices&&(res.responseCommandIndices=data.responseCommandIndices),data.results.push(res)}else{const res={error:Errors.NONE,symbol:command.symbol,value:command.readValue};void 0!==data.response&&(res.response=data.response),void 0!==data.responseCommandIndices&&(res.responseCommandIndices=data.responseCommandIndices),data.results.push(res)}hasCommandError?Callback.callSafeEx(args.error,null,data):Callback.callSafeEx(args.success,null,data),Callback.callSafeEx(args.completed,null,data)}}}TcHmi.Server={...TcHmi.Server??{},getWebsocketReadyState,isWebsocketReady,isReady,getApiVersion,writeSymbol,writeSymbolEx,writeSymbolEx2,readSymbol,readSymbolEx,readSymbolEx2,subscribeSymbol,subscribeSymbolEx,subscribeSymbolEx2,resolveSymbolSchema,resolveSymbolSchemaEx,resolveSymbolMetaData,request,requestEx,subscribe,subscribeEx,unsubscribe,unsubscribeEx,releaseRequest,getCurrentUser,getGroupsOfCurrentUser,getCurrentUserConfig,login,loginEx,loginEx2,logout,logoutEx,logoutEx2,forceLogout,forceLogoutEx,handleResponse,ACCESS,Error};