import{animationProvider}from"../System/AnimationProvider.js";import{Log}from"./Log.js";import{CallbackMethod}from"./Decorators.js";
/**
 ...
 * @deprecated Please use new TcHmi.Animation()
 */TcHmi.AnimationProvider={create:function(controlName,selector=""){return new Animation(controlName,selector)}};let Animation=(()=>{let ___updateState_decorators,_instanceExtraInitializers=[];return class Animation{static{const _metadata="function"==typeof Symbol&&Symbol.metadata?Object.create(null):void 0;___updateState_decorators=[CallbackMethod],__esDecorate(this,null,___updateState_decorators,{kind:"method",name:"__updateState",static:!1,private:!1,access:{has:obj=>"__updateState"in obj,get:obj=>obj.__updateState},metadata:_metadata},null,_instanceExtraInitializers),_metadata&&Object.defineProperty(this,Symbol.metadata,{enumerable:!0,configurable:!0,writable:!0,value:_metadata})}
/**
         * Creates a new animation
         * @param controlName The name of the control for which the animation is intendend.
         * @param selector A CSS selector to identify the element inside the control to animate.
         * @preserve (Part of the public API)
         */constructor(controlName,selector=""){this.__controlName=controlName,this.__selector=selector,this.__animationName=`anim-${tchmi_create_guid()}`}__controlName=__runInitializers(this,_instanceExtraInitializers);__selector;__animationName;__keyframes=[];__duration=0;__delay=0;__iterationCount=1;__direction="normal";__timingFunction="ease";__fillMode="none";__cleanup=!1;__useCss=!0;__eventHandlers=[];__state=Animation.Status.CONFIGURE;__updateState(status){this.__state=status}__animationController=null;
/**
...
/**
         * Run the animation.
         * @preserve (Part of the public API)
         */run(){return 0===this.__keyframes.length?(Log.error("[Source=Framework, Module=TcHmi.Animation] This animation has no keyframes defined."),this):0===this.__keyframes.filter(keyframe=>0===keyframe.progressPoint).length?(Log.error("[Source=Framework, Module=TcHmi.Animation] This animation has no start keyframe with progressPoint 0 defined."),this):0===this.__keyframes.filter(keyframe=>1===keyframe.progressPoint).length?(Log.error("[Source=Framework, Module=TcHmi.Animation] This animation has no end keyframe with progressPoint 1 defined."),this):(null!==this.__animationController&&this.__animationController.isValid()||(this.__animationController=animationProvider.createAnimationController(this,this.__updateState)),this.__animationController.run(),this)}
/**
         * Pause the animation.
         * @preserve (Part of the public API)
...
/**
         ...
         * @preserve (Part of the public API)
         */reset(callback){return null!==this.__animationController&&this.__animationController.reset(callback),this}__canUseCss(){return"string"==typeof this.__timingFunction}__configAllowed(){return this.__state===Animation.Status.CONFIGURE||this.__state===Animation.Status.ENDED||(Log.warn("[Source=Framework, Module=TcHmi.Animation] Configuring an animation is only allowed when it has the state CONFIGURE or ENDED."),!1)}}})();export{Animation};!function(Animation){let Status;!function(Status){Status[Status.CONFIGURE=0]="CONFIGURE",Status[Status.INITIALIZED=1]="INITIALIZED",Status[Status.RUNNING=2]="RUNNING",Status[Status.PAUSED=3]="PAUSED",Status[Status.ENDED=4]="ENDED"}(Status=Animation.Status||(Animation.Status={}))}(Animation||(Animation={}));TcHmi.Animation=Animation;