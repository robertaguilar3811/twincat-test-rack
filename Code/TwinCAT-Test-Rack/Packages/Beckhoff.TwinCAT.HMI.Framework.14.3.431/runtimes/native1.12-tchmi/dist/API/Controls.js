import{mapControlNamesFromPackageManifestApi1ToApi0,injectInGlobalObject,controlRegistrations as registrations}from"../System/RegistrationHelpers.js";export*as System from"../Controls/System/baseTcHmiControl.js";import{Errors}from"./_Types.js";import{EventProvider}from"./EventProvider.js";import{Log}from"./Log.js";let controlManager,getControlBasePath,getControlBasePathEx;
/**
 * DEPRECATED
 * Register a control.
 ...
 */
function register(controlTypeName,constructor,directory,template){let registration={name:controlTypeName,directory,error:Errors.NONE,apiVersion:0,ctrlConstructor:constructor,template};if(constructor.toString().startsWith("class"))registration.nativeEs6Control=!0;else{registration.nativeEs6Control=!1;let firstES6constructor=constructor;do{firstES6constructor=Object.getPrototypeOf(firstES6constructor)}while(firstES6constructor&&!firstES6constructor.toString().startsWith("class"));firstES6constructor&&(registration.nearestEs6Constructor=firstES6constructor)}registrations.map.has(controlTypeName)?registrations.map.set(controlTypeName,{apiVersion:1,error:Errors.E_NOT_UNIQUE,errorDetails:{code:Errors.E_NOT_UNIQUE,message:Errors[Errors.E_NOT_UNIQUE],domain:"TcHmi.Controls.register",reason:'Ambiguous registration for the type name: "'+controlTypeName+'". Control will not be addressable by this type name. Please try fully qualified name to access control.'}}):registrations.map.set(controlTypeName,registration),registrations.array.push(registration),EventProvider.raise("System.onControlRegistered",registration)}
/**
 * Register a control.
 * The Framework will fetch and interprete the Description.json in the Control Directory and optionally load a HTML Template file
 * @param controlTypeName Name of the Control type.
 ...
 * @template C defines the type for the control to register
 * @preserve (Part of the public API)
 */import("../System/ControlManager.js").then(module=>{controlManager=module.controlManager}),import("./Environment.js").then(module=>{getControlBasePath=module.getControlBasePath,getControlBasePathEx=module.getControlBasePathEx});export function registerEx(controlTypeName,namespace,constructor,options){let foundDuplicate=!1,registration={name:controlTypeName,namespace,error:Errors.NONE,apiVersion:1,ctrlConstructor:constructor};if(constructor.toString().startsWith("class"))registration.nativeEs6Control=!0;else{registration.nativeEs6Control=!1;let firstES6constructor=constructor;do{firstES6constructor=Object.getPrototypeOf(firstES6constructor)}while(firstES6constructor&&!firstES6constructor.toString().startsWith("class"));firstES6constructor&&(registration.nearestEs6Constructor=firstES6constructor)}const qname=namespace?namespace+"."+controlTypeName:controlTypeName;if(registrations.map.has(controlTypeName)?registrations.map.set(controlTypeName,{apiVersion:1,error:Errors.E_NOT_UNIQUE,errorDetails:{code:Errors.E_NOT_UNIQUE,message:Errors[Errors.E_NOT_UNIQUE],domain:"TcHmi.Controls.registerEx",reason:`Ambiguous registration for the type name: "${controlTypeName}". Control will not be addressable by this type name. Please try fully qualified name to access control.`}}):registrations.map.set(controlTypeName,registration),registrations.map.has(qname)?(registrations.map.set(qname,{apiVersion:1,error:Errors.E_NOT_UNIQUE,errorDetails:{code:Errors.E_NOT_UNIQUE,message:Errors[Errors.E_NOT_UNIQUE],domain:"TcHmi.Controls.registerEx",reason:`Ambiguous registration for the name: "${qname}". Control will not be usable.`}}),foundDuplicate=!0):(registrations.map.set(qname,registration),options?.injectInGlobalObject&&injectInGlobalObject(qname,constructor)),mapControlNamesFromPackageManifestApi1ToApi0.has(qname)){let nameLegacy=mapControlNamesFromPackageManifestApi1ToApi0.get(qname);registrations.map.has(nameLegacy)?(registrations.map.set(nameLegacy,{apiVersion:1,error:Errors.E_NOT_UNIQUE,errorDetails:{code:Errors.E_NOT_UNIQUE,message:Errors[Errors.E_NOT_UNIQUE],domain:"TcHmi.Controls.registerEx",reason:`Ambiguous registration for the name: "${nameLegacy}". Control will not be addressable by this name.`}}),foundDuplicate=!0):registrations.map.set(nameLegacy,registration)}foundDuplicate||(registrations.array.push(registration),EventProvider.raise("System.onControlRegistered",registration))}
/**
