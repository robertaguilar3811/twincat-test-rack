import{accessManager}from"../System/AccessManager.js";export function __rebuildLocalizationCache(){__numberFormatMilliSecondsCache.clear(),__dateFormatCache.clear()}let __numberFormatMilliSecondsCache=new Map,__dateFormatCache=new Map;
/**
...
/**
     ...
     * @preserve (Part of the public API)
     */
function parseInt(input){let value="";for(let i=0;i<input.length;i++)input.charCodeAt(i)>=32&&input.charCodeAt(i)<=126&&(value+=input[i]);return Number.parseInt(value,10)}Localization.getDateTimeFormatter=getDateTimeFormatter,Localization.parseInt=parseInt,Localization.parseDate=
/**
...
/**
     ...
     * @preserve (Part of the public API)
     */
function(date,options){if(isNaN(date.getTime()))return null;const locale=options?.locale??void 0,timeZone=options?.timeZone,format=options?.type;return getDateTimeFormatter(locale,timeZone,void 0,format).format(date)}}(Localization||(Localization={}));Localization.getDateTimeFormatter,Localization.formatDate,Localization.parseDate,Localization.parseInt;TcHmi.Localization=Localization;