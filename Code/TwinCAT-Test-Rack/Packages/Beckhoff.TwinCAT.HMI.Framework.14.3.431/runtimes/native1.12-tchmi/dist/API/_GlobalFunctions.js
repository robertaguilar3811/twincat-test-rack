 ...
 */
function _tchmi_equal(a,b,typeSafeOrOptions=!0){if(a===b)return!0;if(null===a||null===b)return!1;let options={convertPrimitives:!1,compareDates:!1};"boolean"==typeof typeSafeOrOptions?options.convertPrimitives=!typeSafeOrOptions:(options.convertPrimitives=typeSafeOrOptions.convertPrimitives??!1,options.compareMaps=typeSafeOrOptions.compareMaps,options.compareSets=typeSafeOrOptions.compareSets,options.compareDates=typeSafeOrOptions.compareDates??!1);let typeA=typeof a,typeB=typeof b;if(typeA!==typeB){if(options.convertPrimitives)switch(typeA){case"boolean":switch(typeB){case"number":return a==b;case"string":return a.toString()===b.toLowerCase()}break;case"number":switch(typeB){case"boolean":return a==b;case"string":return""!==b&&a==b}break;case"string":switch(typeB){case"boolean":return a.toLowerCase()===b.toString();case"number":return""!==a&&a==b}}return!1}if("number"===typeA&&isNaN(a)&&isNaN(b))return!0;if(a instanceof Map&&b instanceof Map&&options.compareMaps){if(a.size!==b.size)return!1;const aIterator=a.entries(),bIterator=b.entries();let aResult=aIterator.next(),bResult=bIterator.next();for(;!aResult.done&&!bResult.done;){const[aKey,aValue]=aResult.value,[bKey,bValue]=bResult.value;if(options.compareMaps.deepCompareKeys){if(!_tchmi_equal(aKey,bKey,typeSafeOrOptions))return!1}else if(aKey!==bKey)return!1;if(options.compareMaps.deepCompareValues){if(!_tchmi_equal(aValue,bValue,typeSafeOrOptions))return!1}else if(aValue!==bValue)return!1;aResult=aIterator.next(),bResult=bIterator.next()}return!0}if(a instanceof Set&&b instanceof Set&&options.compareSets){if(a.size!==b.size)return!1;const aIterator=a.values(),bIterator=b.values();let aResult=aIterator.next(),bResult=bIterator.next();for(;!aResult.done&&!bResult.done;){if(options.compareSets.deepCompareValues){if(!_tchmi_equal(aResult.value,bResult.value,typeSafeOrOptions))return!1}else if(aResult.value!==bResult.value)return!1;aResult=aIterator.next(),bResult=bIterator.next()}return!0}if(a instanceof Date&&b instanceof Date&&options.compareDates)return a.valueOf()===b.valueOf();if("object"!==typeA||__tchmi_is_instanced_object(a)||__tchmi_is_instanced_object(b))return!1;let aIsArray=Array.isArray(a);if(aIsArray!==Array.isArray(b))return!1;if(aIsArray){let length=a.length;if(length!==b.length)return!1;for(let i=0;i<length;i++)if(!_tchmi_equal(a[i],b[i],typeSafeOrOptions))return!1;return!0}if(Object.keys(a).length!==Object.keys(b).length)return!1;for(let key in a){if(!b.hasOwnProperty(key))return!1;if(!_tchmi_equal(a[key],b[key],typeSafeOrOptions))return!1}return!0}window.tchmi_decode_control_characters=
/**
 ...
 * @preserve (Part of the public API)
 */,window.tchmi_encode_control_characters=function(text){if(!text)return"";if("string"!=typeof text){if("function"!=typeof text.toString)return text;text=text.toString()}return text.replace(/\\/gm,"\\\\").replace(/\n/gm,"\\n").replace(/\r/gm,"\\r").replace(/\t/gm,"\\t")}
/**
 * Encodes HTML
 * @param html The html to encode
 * @returns The encoded representation of parameter html.
 * @preserve (Part of the public API)
 */,window.tchmi_encode_html=function(html){"string"!=typeof html&&"function"==typeof html.toString&&(html=html.toString());return String(html).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}
/**
 * Decodes HTML
 ...
 * "°     !\"§     $%&/()=?~#|é     è     €…™®     ©"
 * @preserve (Part of the public API)
 */,window.tchmi_decode_html=function(text){"string"!=typeof text&&"function"==typeof text.toString&&(text=text.toString());return String(text).replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&#228;/g,"ä").replace(/&#196;/g,"Ä").replace(/&#246;/g,"ö").replace(/&#214;/g,"Ö").replace(/&#252;/g,"ü").replace(/&#220;/g,"Ü").replace(/&#223;/g,"ß").replace(/&#233;/g,"é").replace(/&#232;/g,"è").replace(/&#176;/g,"°").replace(/&#167;/g,"§").replace(/&#174;/g,"®").replace(/&#169;/g,"©").replace(/&#181;/g,"µ")}
/**
 ...
 * @preserve (Part of the public API)
 */,window.tchmi_base64decode=function(input){"string"!=typeof input&&"function"==typeof input.toString&&(input=input.toString());return window.atob(input)}
/**
 ...
 * @returns Returns the String or null
 * @preserve (Part of the public API)
 */,window.tchmi_base64encode=function(input){"string"!=typeof input&&"function"==typeof input.toString&&(input=input.toString());return window.btoa(input)}
/**
 * Creates a Guid string.
 * @returns Guid
 * @preserve (Part of the public API)
 */,window.tchmi_create_guid=function(){if(window.crypto.randomUUID)return window.crypto.randomUUID();let d=performance.now();return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(c){let r=(d+16*Math.random())%16|0;return d=Math.floor(d/16),("x"===c?r:7&r|8).toString(16)})},window.tchmi_equal=_tchmi_equal,window.tchmi_clone_object=
/**
